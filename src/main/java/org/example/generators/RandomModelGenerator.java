...
public class RandomModelGenerator {
        ...
        try {
            ...
            for (Field field : getAllFields(clazz)) {
                ...
                if (rule != null) {
                    ...
                    if (type.isEnum()) {
                        // Пытаемся найти enum-константу с именем, равным regex
                        try {
                            // Небезопасное приведение, но мы знаем, что type - enum
                            @SuppressWarnings({"unchecked", "rawtypes"})
                            Object enumConstant = Enum.valueOf((Class<Enum>) type, rule.regex());
                            value = enumConstant;
                        } catch (IllegalArgumentException e) {
                            // Если константа не найдена, генерируем случайное значение enum
                            value = getRandomEnumValue(type);
                        }
                    } else {
                        value = generateFromRegex(rule.regex(), type);
                    }
                } else {
    ...
    private static Object getRandomEnumValue(Class<?> enumClass) {
        Object[] enumConstants = enumClass.getEnumConstants();
        if (enumConstants == null || enumConstants.length == 0) {
            throw new IllegalArgumentException("Enum " + enumClass + " has no constants");
        }
