...
public class TransferTest extends BaseTest {
    ...
    public void positiveTransferTest(Double balance, int receiverAccountId, Double amount) {
        ...
        softAssertions.assertThat(getAccountById(userAccounts.get(0).getId()).getTransactions().size()).as("Количество транзакций не совпадает с ожидаемым").isEqualTo((int) (balance / 5000) + 1 + beginUser1Account1Transactions + 1);

        AccountDAO accountDao1 = DataBaseSteps.getAccountByAccountNumber(userAccounts.get(0).getAccountNumber());
        DaoAndModelAssertions.assertThat(getAccountById(userAccounts.get(0).getId()), accountDao1).match();
        ...
        AccountDAO accountDao2 = DataBaseSteps.getAccountByAccountNumber(userAccounts.get(receiverAccountId).getAccountNumber());
        DaoAndModelAssertions.assertThat(getAccountById(userAccounts.get(receiverAccountId).getId()), accountDao2).match();
    }
